...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logging.info(str(os.environ["FORMS_RECOGNIZER_ENDPOINT"]))
    try:
        ...
        return func.HttpResponse(   
             "Skill configuration error. Endpoint, key and model_id required." + str(e),
             status_code=400
        )
...
def compose_response(json_data):
    ...
    document_analysis_client = DocumentAnalysisClient(endpoint=os.environ["FORM_RECOGNIZER_ENDPOINT"], credential=credential)

    for value in values:
        output_record = transform_value(value, mappings, document_analysis_client, model_id)
